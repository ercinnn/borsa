...
/// TechnicalWatchlistStore/DividendWatchlistStore ile aynı desen —
/// watchlist/favorites/teknik/temettü listelerinden bağımsız, bildirim
/// üretmez. Önceden Temel Analiz, TechnicalWatchlistStore'u doğrudan
/// paylaşıyordu (Teknik'e eklenen bir sembol otomatik burada da
/// görünüyordu); kullanıcı isteğiyle bu iki sekme birbirinden ayrıldı —
/// bkz. supabase_schema_fundamentals_watchlist.sql'deki bir kerelik geriye
/// dönük kopyalama notu (mevcut kullanıcılar geçişte Temel Analiz'de
/// gördükleri sembolleri kaybetmesin diye).
class FundamentalsWatchlistStore {
  final SupabaseTable _table;

  FundamentalsWatchlistStore(SupabaseConfig config, http.Client client)
      : _table = SupabaseTable(client, config, 'fundamentals_watchlist');
  ...
  /// fundamentals_cache.dart'ın arka plan ön-senkronizasyonu için: tüm
  /// kullanıcıların tüm sembolleri — TechnicalWatchlistStore.allRows() ile
  /// aynı desen (önceden syncWatchlistedSymbols bunun yerine
  /// TechnicalWatchlistStore.allRows()'u kullanıyordu, artık kendi listesini
  /// takip ediyor).
  Future<List<Map<String, dynamic>>> allRows() {
    return _table.select(columns: 'user_id,symbol');
  }
